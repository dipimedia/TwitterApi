function muestraMensjaes(){
	...
	$('#respuesta').find('#resp').on('click',function () {
		alert('hola');
	});	

}

function respondeMensaje(e){
	...
	$.get('api.php?funcion=mensajes',function (data){
		...
		if (datos.length > 0 ){
			...
			$.each(datos, function(index, valor) {
			    ...
			    pagina = '';
				pagina += '<a href="#" class="list-group-item">';
				pagina += '<img src="' + valor.avatar + '" alt="@"' + valor.de + ' class="img-circle">';
				pagina += '<h4 class="list-group-item-heading">@<span class="user">' + valor.de + '</span></h4>';
				pagina += '<p class="list-group-item-text">' + valor.texto + '</p>';
				pagina += '<button type="button" id="resp" class="btn btn-primary">Responder</button></a>';
	...
	$.get('api.php?funcion=seguidores',function (data){
		...
		if (datos.length > 0 ){
			...
			$.each(datos, function(index, valor) {
				...
				pagina += '<h4 class="list-group-item-heading">@<span class="user">' + valor.usuario + '</span></h4>';
